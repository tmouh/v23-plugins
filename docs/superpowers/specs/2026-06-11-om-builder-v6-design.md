...
**Scope (per Theo 2026-06-11): learn more than the voice.** The sweep takes structured notes on every dimension worth systematizing, and all raw notes persist to a study workspace on disk (`study/om-pattern-study-2026-06/` in this repo, gitignored if the repo has a remote) so the study is resumable and auditable. Dimensions: voice/diction; layout & composition; document structure by register; exec-summary construction; headline/title grammar; KPI & table presentation; number, unit, and citation conventions; risk framing; sponsor/track-record presentation; map/photo/visual usage; density and pacing norms; cover/disclaimer/contact conventions; and the negative space — what V23 materials conspicuously never do, learned from the materials themselves.

**Artifacts** (live in the skill folder; each carries a "regenerate by re-running the sweep" note; final set may be consolidated during synthesis):
- `voice-model.md` — deal-agnostic diction, rhythm, headline grammar, and prose patterns; every pattern cited to ≥3 materials spanning years. Replaces house-voice Part 1. The anti-AI-tell ruleset (current Part 2) is retained as the backstop — it is external research by design — and is extended with the observed V23 negative-space list.
- `layout-system.md` — full overhaul of layout-repertoire: arrangements actually observed across V23 decks (extracted from shape geometry at scale + selective renders), the content→layout selection logic, and composition/density norms. The v5.1 selection research is folded in where it survives contact with the observed decks.
- `design-system.md` — the visual DNA stable across decks (typography behavior, palette, footer/source-line conventions), explicitly separating durable house DNA from per-deck choices. Supersedes the hardcoded style block in prompt-template.md.
- `registers-and-coverage.md` — how equity / debt / pref / JV materials differ in structure, emphasis, and metrics shown (from 105 N 13 vDebt, Wind Gap Pref and JV OMs, lender-facing files, and every other register instance found). Checklists, not templates.
- `conventions.md` — numbers, units, decimals, negatives, footnote and source-line style, exec-summary construction, sponsor presentation, cover/disclaimer/contact patterns — as observed, not assumed.
