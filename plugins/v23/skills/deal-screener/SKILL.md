---
name: deal-screener
description: "Screen commercial real estate Offering Memorandums (OMs) and deal teasers with the eye of a senior real estate / capital-markets executive. Use this skill whenever the user shares an OM, a deal teaser, an Investment Committee memo, an underwriting model, a pitch deck, or any document where a sponsor / broker is presenting a CRE deal — even if the user just says 'screen this,' 'what do you think of this deal,' 'review this OM,' 'pull this apart,' 'kick the tires on this,' or pastes a property name with financials. Trigger across all asset classes (multifamily, office, retail, industrial, hospitality, self-storage, data center, MHC, mixed-use, land), all seats (equity placement, debt placement, principal acquisition, LP deployment), and all stages (initial screen, IC memo, broker comp, refi diligence). Produces a structured deal-screening report with a forensic financial review, market check, sponsor read, risk register, ranked questions for the broker/sponsor, and a pursue/pass/conditional recommendation. Improves over time via a lessons-learned file the skill consults on every run."
---
