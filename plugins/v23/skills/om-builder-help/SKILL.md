...
4. **Python 3 with `python-pptx`** — used by `verify-deck.py`, `build-deck.py`, `apply-revision-edits.py`, and `make-template.py`; `clone-build.py` uses `lxml`, which python-pptx installs as a dependency. (`extract-pptx-inventory.py` and `render-qa.py` are stdlib-only.)
...
### Phase 4 — Build (CC-native, clone-first as of v6.1)

**Default method: seed-deck XML clone via `clone-build.py`.** A register-matched production deck (debt OM → the 105 N 13 vDebt deck; equity full OM → the 105 N 13 final; platform → the NPV final; or any newer send-state deck of the matching register) is frozen at `x V23\_build\seeds\`, inventoried, and cloned slide-by-slide; content is edited surgically at the XML run/cell level from a deal-local buildspec (precondition-checked text, table rebuilds from row templates, image swaps with center-crop, orphan-part prune so the seed's media never ships). Structure and formatting are inherited from real production slides, never drawn from primitives. The v6.0 generator-first path was demoted after its first live run shipped a rejected sparse deck — `assets\v23-template.pptx` carries zero slides, so the generator can only draw thin primitives. `build-deck.py` remains the fallback when no register-matched seed exists. Every authored sentence is governed by `voice-model.md` (GP rules outrank everything) + `anti-ai-ruleset.md` (read-aloud pass); layout selection by `layout-system.md` (structural-first); number formatting, typography, and footers by `design-system.md` + `conventions.md`. A build log records every slide's seed source, arrangement key, edits, and prune count.
...
- **5b Verification:** every deck numeric is re-checked against the audit trail. Criticals (ask size, key returns, cap stack) re-check directly to the source file/tab/page; research claims re-fetch the cited URL. Any figure without a ✅ entry is replaced with "TBD — confirm with sponsor" before shipping. As of v6.1 the re-check is codified: `verify-deck.py <deck.pptx> <checks.py>` runs MUST-PRESENT regexes (from the ledger) and MUST-ABSENT regexes (from the blueprint stop-list + seed-deck remnants) over the extracted deck text; its output is the 5b evidence filed with the QA report.
...
| Script | Role |
|---|---|
| `scripts/clone-build.py` | Phase 4 DEFAULT build engine (v6.1): buildspec-driven XML surgery on a register-matched seed deck — slide plan (keep/clone/delete/reorder), precondition-checked text replacement with inherited formatting, table rebuilds from row templates, image swaps with center-crop, orphan-part prune. Trap list in its docstring |
| `scripts/verify-deck.py` | Phase 5b verification harness: MUST-PRESENT (ledger) + MUST-ABSENT (stop-list + seed remnants) regex checks over extracted deck text; exit code wired for the QA loop |
| `scripts/build-deck.py` | Phase 4 FALLBACK generator (no register-matched seed): JSON slide spec → `.pptx` on `assets/v23-template.pptx` (a zero-slide master shell). Deterministic layouts: cover, kpi_strip, narrative, table, chart, two_column, section_divider. Output is structurally sparse — expect heavy Phase 5a work; any unsupported layout becomes a labeled placeholder slide |
...
- **The clone path needs a register-matched seed.** `clone-build.py` inherits everything from a real production deck of the same register; if none exists (a brand-new register), the `build-deck.py` fallback generates structurally sparse output that needs heavy Phase 5a work — full-bleed heroes, aerials, maps, and renderings ship as labeled placeholder slides routed to CIP polish or manual finish. Build the first deck of a new register expecting that cost; every deck after it has a seed.
